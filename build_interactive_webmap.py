...
DEFAULT_OUTPUT_DIR = "felt-gw-graffiti-assets/map"


WEBMAP_HTML = r"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    .stat { background: white; border: 1px solid #d9e2ec; border-radius: 6px; padding: 10px; }
...
<body>
  ...
  <script>
    ...
    function matches(feature) {
      ...
      if (q) {
        const haystack = [
          p.job_id, p.asset_name, p.located_on, p.relative_location, p.council,
          p.assigned_to, p.priority, p.recent_comment, p.defect_description
...
def bool_text(value):
    return "TRUE" if str(value or "").strip().upper() == "TRUE" else "FALSE"
    ...
    with open(csv_path, newline="", encoding="utf-8-sig") as input_file:
        for row in csv.DictReader(input_file):
            ...
            for key, value in row.items():
                props[key] = value
            props["Can GW Access"] = bool_text(row.get("Can GW Access"))
            props["Before 2025 EOFY"] = bool_text(row.get("Before 2025 EOFY"))
            try:
...
def write_qgis_hint(output_dir):
    text = """# QGIS Notes
...
```
"""
    with open(os.path.join(output_dir, "QGIS_NOTES.md"), "w", encoding="utf-8") as output:
        output.write(text)


def main():
    parser = argparse.ArgumentParser(description="Build an interactive static web map from graffiti_jobs.csv.")
    ...
    geojson = build_geojson(csv_path)
    with open(os.path.join(data_dir, "jobs.geojson"), "w", encoding="utf-8") as output:
    ...
    with open(os.path.join(output_dir, "index.html"), "w", encoding="utf-8") as output:
        output.write(WEBMAP_HTML)
    write_qgis_hint(output_dir)
    print(f"Wrote {len(geojson['features'])} mapped jobs to {output_dir}")
